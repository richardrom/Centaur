...
struct CDialog::Impl
{
    const CENTAUR_THEME_INTERFACE_NAMESPACE::DialogInformation *uiInformation { nullptr };
...
CDialog::CDialog(QWidget *parent) :
    QDialog { parent },
    _impl { std::make_unique<CDialog::Impl>() }
{
...
void CDialog::paintEvent(QPaintEvent *event)
{
    ...
    if (P_IMPL()->uiInformation == nullptr) {
        const auto thisObjectName = this->objectName();
        const auto iter           = cuiTheme()->uiElements().dialogOverride.find(thisObjectName);
        if (iter != cuiTheme()->uiElements().dialogOverride.end()) {
            P_IMPL()->uiInformation = std::addressof(iter->second);
        }
        else
            P_IMPL()->uiInformation = std::addressof(cuiTheme()->uiElements().dialogInformation);
